...
- セキュリティ対策として reCAPTCHA 導入（ユーザー登録・パスワード再発行）
- パスワード再発行メール送信機能の本番対応（SendGrid 経由）
- 本番環境 `.env.production` を整理し、運用構成を明確化

### 0.9.0（2025/06/06）

- EC2 への初回デプロイ完了（HTTPS 対応含む）
- nginx + Gunicorn + systemd による常駐運用構成
- ToDo アプリの基本機能を実装（タスク登録、完了、削除など）
...
- 緊急度・重要度の設定（選択式）
- タスク一覧画面に、緊急度高の場合は！、重要度高の場合は ★ のアイコンを表示
- 目標・実績の入力（任意）
...
- SendGridによりパスワード再発行時のメールの送信元のアドレスを設定
